...
app.get('/list', (req, res) => {
    ...
    requestchange.count((err, counts) => {
        if (counts) {
            htmlRe = htmlRe + "<table class='table col-sm-12'><thead><tr><th scope='col'>申请者</th><th scope='col'>联系方式</th><th scope='col'>申请住宅</th><th scope='col'>申请内容</th><th scope='col'>处理方式</th><th scope='col'>处理方式</th></tr></thead><tbody>"
            requestchange.find({}, null, { limit: 5 }, (err, result) => {
                result.forEach((value, index) => {
                    ...
                    htmlRe = htmlRe + "<td id='location"+index+"'>"+value.location+"幢"+value.houseNumber+"室</td>"
